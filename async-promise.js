...
// the good solution is to write the code in promise and in async await
// the better way is to use the 'util' module
// But the best way is to use the '.promises' after the file('fs') module
...
const start = async () => {
    try {
        const first = await readFile('./content/async.txt', 'utf8')
        const second = await readFile('./content/subfolder/app.text', 'utf8')
        await writeFile('./content/mind-granade.txt',
